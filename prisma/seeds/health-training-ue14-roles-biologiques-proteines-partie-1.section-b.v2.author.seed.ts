import type { HealthTrainingAuthorQuestion } from './health-training-ue14.shared';

export const UE14_BIOCH_CH12_SECTION_B_V2: HealthTrainingAuthorQuestion[] = [
  {
    "order": 23,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Concernant l’architecture générale d’un récepteur membranaire :",
    "choices": [
      {
        "content": "La fiche présente un domaine extracellulaire de reconnaissance du ligand.",
        "correct": true,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Tous les récepteurs sont entièrement extracellulaires.",
        "correct": false,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Un domaine transmembranaire traverse la membrane.",
        "correct": true,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Le domaine cytoplasmique sert uniquement à reconnaître les ligands circulants.",
        "correct": false,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      }
    ],
    "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
  },
  {
    "order": 24,
    "difficulty": "EASY",
    "format": "QRU",
    "question": "Quelle proposition décrit correctement les récepteurs nucléaires selon la fiche ?",
    "choices": [
      {
        "content": "La fiche les présente comme une large famille de plus de 150 protéines.",
        "correct": true,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "Ils sont tous localisés dans la matrice extracellulaire.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "La fiche ne leur attribue aucun ligand.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "Les récepteurs nucléaires sont exclusivement des canaux ioniques membranaires.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      }
    ],
    "explanation": "Les récepteurs nucléaires sont intracellulaires."
  },
  {
    "order": 25,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Repères fondamentaux sur les ligands des récepteurs nucléaires :",
    "choices": [
      {
        "content": "L’acide rétinoïque et la vitamine D sont également cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "La fiche exclut les hormones stéroïdiennes de cette famille.",
        "correct": false,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "Les hormones thyroïdiennes font partie des ligands cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "Le glucose est présenté comme l’unique ligand des récepteurs nucléaires.",
        "correct": false,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      }
    ],
    "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
  },
  {
    "order": 26,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Parmi les propositions suivantes sur le fonctionnement transcriptionnel des récepteurs nucléaires, laquelle est exacte ?",
    "choices": [
      {
        "content": "Après fixation du ligand, des récepteurs nucléaires peuvent se dimériser.",
        "correct": true,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Leur activation ouvre directement un canal Na+.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Ils se lient ensuite à l’ADN dans le noyau.",
        "correct": true,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Ils hydrolysent obligatoirement l’ATP pour transporter leur ligand.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      }
    ],
    "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
  },
  {
    "order": 27,
    "difficulty": "EASY",
    "format": "QRU",
    "question": "Concernant les récepteurs métabotropiques :",
    "choices": [
      {
        "content": "La transmission fait intervenir un second messager chimique ou enzymatique.",
        "correct": true,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Aucun second messager n’est impliqué dans la signalisation métabotropique.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Un récepteur métabotropique est forcément un récepteur nucléaire.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Les fonctions réceptrice et effectrice sont toujours assurées par le même canal ionique.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      }
    ],
    "explanation": "Le support oppose métabotropiques et ionotropiques."
  },
  {
    "order": 28,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Quelle proposition décrit correctement la structure du récepteur à l’insuline selon la fiche ?",
    "choices": [
      {
        "content": "La chaîne β porte un domaine transmembranaire et un domaine tyrosine kinase cytoplasmique.",
        "correct": true,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "Le récepteur à l’insuline est un monomère soluble.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "Le récepteur à l’insuline est un hétérotétramère comportant des chaînes α et β.",
        "correct": true,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "La chaîne α traverse sept fois la membrane.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      }
    ],
    "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
  },
  {
    "order": 29,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Repères fondamentaux sur la voie du récepteur à l’insuline :",
    "choices": [
      {
        "content": "La fixation de l’insuline précède la phosphorylation de tyrosines.",
        "correct": true,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "La voie citée débute par l’ouverture directe d’un canal Cl−.",
        "correct": false,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "La fiche cite la voie IRS/PI3K/PIP3/PKB.",
        "correct": true,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "La fiche exclut PI3K de la signalisation insulinique.",
        "correct": false,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      }
    ],
    "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
  },
  {
    "order": 30,
    "difficulty": "EASY",
    "format": "QRU",
    "question": "Parmi les propositions suivantes sur les récepteurs couplés aux protéines G, laquelle est exacte ?",
    "choices": [
      {
        "content": "La première étape de signalisation intracellulaire implique l’activation d’une protéine G.",
        "correct": true,
        "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
      },
      {
        "content": "Ils fonctionnent uniquement grâce à une pompe Na+/K+.",
        "correct": false,
        "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
      },
      {
        "content": "Ils sont décrits comme des immunoglobulines pentamériques.",
        "correct": false,
        "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
      },
      {
        "content": "Les RCPG ne traversent la membrane qu’une seule fois.",
        "correct": false,
        "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
      }
    ],
    "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
  },
  {
    "order": 31,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Concernant la rhodopsine :",
    "choices": [
      {
        "content": "Un photon provoque une isomérisation du rétinal et l’activation de la transducine.",
        "correct": true,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "La lumière n’entraîne aucun changement de conformation de la rhodopsine.",
        "correct": false,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "La rhodopsine est une protéine des bâtonnets de la rétine.",
        "correct": true,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "La rhodopsine est une pompe Na+/K+ musculaire.",
        "correct": false,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      }
    ],
    "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
  },
  {
    "order": 32,
    "difficulty": "EASY",
    "format": "QRM",
    "question": "Quelle proposition décrit correctement les récepteurs opioïdes dans la fiche selon la fiche ?",
    "choices": [
      {
        "content": "La fiche cite trois récepteurs opioïdes notés μ, γ et κ.",
        "correct": true,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "La fiche cite uniquement un récepteur opioïde β.",
        "correct": false,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "Ils sont rattachés à la famille des RCPG.",
        "correct": true,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "Les récepteurs opioïdes sont décrits comme des récepteurs nucléaires de la vitamine D.",
        "correct": false,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      }
    ],
    "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
  },
  {
    "order": 33,
    "difficulty": "MEDIUM",
    "format": "QRM",
    "question": "Application : quelles propositions relatives à les ligands des récepteurs nucléaires sont correctes ?",
    "choices": [
      {
        "content": "Les hormones stéroïdiennes et le cortisol sont cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "Les photons sont les seuls ligands de tous les récepteurs nucléaires.",
        "correct": false,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "L’acide rétinoïque et la vitamine D sont également cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "La fiche exclut les hormones stéroïdiennes de cette famille.",
        "correct": false,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      }
    ],
    "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
  },
  {
    "order": 34,
    "difficulty": "MEDIUM",
    "format": "QRU",
    "question": "Dans une situation d’application, quelle affirmation sur le fonctionnement transcriptionnel des récepteurs nucléaires faut-il retenir ?",
    "choices": [
      {
        "content": "Ils agissent comme facteurs de transcription.",
        "correct": true,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Ils ne peuvent jamais interagir avec l’ADN.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Leur activation ouvre directement un canal Na+.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Ils hydrolysent obligatoirement l’ATP pour transporter leur ligand.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      }
    ],
    "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
  },
  {
    "order": 35,
    "difficulty": "MEDIUM",
    "format": "QRM",
    "question": "Pour raisonner à partir de la fiche, quelles affirmations sur les récepteurs métabotropiques sont exactes ?",
    "choices": [
      {
        "content": "Les fonctions réceptrice et effectrice sont portées par des molécules différentes.",
        "correct": true,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Les fonctions réceptrice et effectrice sont toujours assurées par le même canal ionique.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "La transmission fait intervenir un second messager chimique ou enzymatique.",
        "correct": true,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Aucun second messager n’est impliqué dans la signalisation métabotropique.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      }
    ],
    "explanation": "Le support oppose métabotropiques et ionotropiques."
  },
  {
    "order": 36,
    "difficulty": "MEDIUM",
    "format": "QRU",
    "question": "Une cellule reçoit un signal insulinique et phosphoryle des tyrosines sur son récepteur. quelle proposition sur la structure du récepteur à l’insuline est correcte ?",
    "choices": [
      {
        "content": "La chaîne α est extracellulaire.",
        "correct": true,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "La chaîne β est uniquement extracellulaire et sans activité enzymatique.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "Le récepteur à l’insuline est un monomère soluble.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "La chaîne α traverse sept fois la membrane.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      }
    ],
    "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
  },
  {
    "order": 37,
    "difficulty": "MEDIUM",
    "format": "QRM",
    "question": "Dans une situation d’application, quelle affirmation sur la voie du récepteur à l’insuline faut-il retenir ?",
    "choices": [
      {
        "content": "Le signal aboutit notamment à une modulation du métabolisme énergétique et de la survie cellulaire.",
        "correct": true,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "Le récepteur à l’insuline ne déclenche aucune phosphorylation.",
        "correct": false,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "La fixation de l’insuline précède la phosphorylation de tyrosines.",
        "correct": true,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "La voie citée débute par l’ouverture directe d’un canal Cl−.",
        "correct": false,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      }
    ],
    "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
  },
  {
    "order": 38,
    "difficulty": "MEDIUM",
    "format": "QROC",
    "question": "Dans la fiche locale, quelle lettre grecque est citée entre μ et κ pour les récepteurs opioïdes ?",
    "answer": {
      "type": "text",
      "acceptedAnswers": [
        "gamma",
        "γ",
        "Gamma"
      ],
      "normalization": {
        "trim": true,
        "caseSensitive": false,
        "collapseWhitespace": true,
        "ignoreAccents": true
      }
    },
    "explanation": "La fiche locale cite μ, γ et κ ; on conserve cette formulation sans la remplacer."
  },
  {
    "order": 39,
    "difficulty": "MEDIUM",
    "format": "QRM",
    "question": "Dans un bâtonnet rétinien exposé à un photon, le 11-cis-rétinal change de configuration. quelles propositions sur la rhodopsine sont conformes à la fiche ?",
    "choices": [
      {
        "content": "Elle est liée au 11-cis-rétinal.",
        "correct": true,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "Le rétinal est présenté comme un dérivé de la vitamine C.",
        "correct": false,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "Un photon provoque une isomérisation du rétinal et l’activation de la transducine.",
        "correct": true,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      },
      {
        "content": "La lumière n’entraîne aucun changement de conformation de la rhodopsine.",
        "correct": false,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      }
    ],
    "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
  },
  {
    "order": 40,
    "difficulty": "MEDIUM",
    "format": "QRU",
    "question": "Dans une situation d’application, quelle affirmation sur les récepteurs opioïdes dans la fiche faut-il retenir ?",
    "choices": [
      {
        "content": "Le support les relie aux hormones peptidiques opioïdes issues du système pro-opiomélanocortine.",
        "correct": true,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "Le support les classe comme pompes ATP-dépendantes.",
        "correct": false,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "La fiche cite uniquement un récepteur opioïde β.",
        "correct": false,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      },
      {
        "content": "Les récepteurs opioïdes sont décrits comme des récepteurs nucléaires de la vitamine D.",
        "correct": false,
        "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
      }
    ],
    "explanation": "Le support local emploie les notations μ, γ et κ ; cette terminologie est conservée telle quelle."
  },
  {
    "order": 41,
    "difficulty": "MEDIUM",
    "format": "QRM",
    "question": "Pour raisonner à partir de la fiche, quelles affirmations sur l’architecture générale d’un récepteur membranaire sont exactes ?",
    "choices": [
      {
        "content": "La fiche présente un domaine extracellulaire de reconnaissance du ligand.",
        "correct": true,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Tous les récepteurs sont entièrement extracellulaires.",
        "correct": false,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Un domaine transmembranaire traverse la membrane.",
        "correct": true,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "Le domaine cytoplasmique sert uniquement à reconnaître les ligands circulants.",
        "correct": false,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      }
    ],
    "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
  },
  {
    "order": 42,
    "difficulty": "MEDIUM",
    "format": "QRU",
    "question": "Application : quelles propositions relatives à les récepteurs nucléaires sont correctes ?",
    "choices": [
      {
        "content": "La fiche les présente comme une large famille de plus de 150 protéines.",
        "correct": true,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "Ils sont tous localisés dans la matrice extracellulaire.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "La fiche ne leur attribue aucun ligand.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "Les récepteurs nucléaires sont exclusivement des canaux ioniques membranaires.",
        "correct": false,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      }
    ],
    "explanation": "Les récepteurs nucléaires sont intracellulaires."
  },
  {
    "order": 43,
    "difficulty": "MEDIUM",
    "format": "QRP",
    "question": "Autour de l’architecture générale d’un récepteur membranaire : sélectionnez exactement les trois affirmations correctes parmi les propositions suivantes.",
    "requiredSelectionCount": 3,
    "choices": [
      {
        "content": "La fiche présente un domaine extracellulaire de reconnaissance du ligand.",
        "correct": true,
        "explanation": "Le schéma général distingue domaines extracellulaire, transmembranaire et cytoplasmique."
      },
      {
        "content": "La fiche les présente comme une large famille de plus de 150 protéines.",
        "correct": true,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "L’acide rétinoïque et la vitamine D sont également cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "Leur activation ouvre directement un canal Na+.",
        "correct": false,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Aucun second messager n’est impliqué dans la signalisation métabotropique.",
        "correct": false,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "Le récepteur à l’insuline est un monomère soluble.",
        "correct": false,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      }
    ],
    "explanation": "Cette question croise plusieurs repères de la section ; chaque proposition doit être évaluée séparément."
  },
  {
    "order": 44,
    "difficulty": "MEDIUM",
    "format": "QRP",
    "question": "Consolidation de l’architecture générale d’un récepteur membranaire : sélectionnez exactement les cinq affirmations correctes.",
    "requiredSelectionCount": 5,
    "choices": [
      {
        "content": "Les récepteurs nucléaires sont localisés dans le noyau et le cytosol.",
        "correct": true,
        "explanation": "Les récepteurs nucléaires sont intracellulaires."
      },
      {
        "content": "Les hormones stéroïdiennes et le cortisol sont cités.",
        "correct": true,
        "explanation": "La fiche cite plusieurs ligands lipophiles ou dérivés vitaminiques."
      },
      {
        "content": "Ils agissent comme facteurs de transcription.",
        "correct": true,
        "explanation": "Le ligand conduit à une action transcriptionnelle nucléaire."
      },
      {
        "content": "Les fonctions réceptrice et effectrice sont portées par des molécules différentes.",
        "correct": true,
        "explanation": "Le support oppose métabotropiques et ionotropiques."
      },
      {
        "content": "La chaîne α est extracellulaire.",
        "correct": true,
        "explanation": "Le récepteur à l’insuline associe chaînes α extracellulaires et β transmembranaires à activité tyrosine kinase."
      },
      {
        "content": "Le récepteur à l’insuline ne déclenche aucune phosphorylation.",
        "correct": false,
        "explanation": "La fiche décrit une cascade IRS/PI3K/PIP3/PKB après activation du récepteur."
      },
      {
        "content": "Les RCPG ne traversent la membrane qu’une seule fois.",
        "correct": false,
        "explanation": "Les RCPG constituent une vaste famille de récepteurs à sept traversées."
      },
      {
        "content": "Le rétinal est présenté comme un dérivé de la vitamine C.",
        "correct": false,
        "explanation": "La rhodopsine illustre un RCPG sensoriel activé par la lumière."
      }
    ],
    "explanation": "La sélection longue vérifie la maîtrise simultanée de plusieurs notions du chapitre."
  }
];
