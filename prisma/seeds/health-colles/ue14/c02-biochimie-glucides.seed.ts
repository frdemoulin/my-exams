...
export async function seedHealthColleUE14C02(prisma: PrismaClient) {
  ...
  if (existingColle) {
    ...
    if (attemptCount > 0) {
      console.warn(`[SEED C02] ${attemptCount} tentative(s) conservée(s). Mise à jour du contenu des questions en place.`);
      const section = existingColle.sections[0];
      if (!section) throw new Error("Section introuvable pour C02.");
  ...
  // Création complète si pas de tentative ou pas d'examen existant
