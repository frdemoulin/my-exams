...
export async function seedHealthColleUE14C11(prisma: PrismaClient) {
  ...
  if (!courseUnit) throw new Error("Impossible de trouver l'UE14 pour C11.");
  ...
  if (existingColle) {
    ...
    if (attemptCount === 0) {
      ...
      console.warn(`[SEED C11] ${attemptCount} tentative(s) conservée(s). Mise à jour du contenu.`);
  ...
  const groupsData = [
    ...
    },
    {
      ...
      order: 2,
      title: "Données communes — Cinétique enzymatique expérimentale",
      sharedStatement:
        "Une enzyme michaelienne est étudiée par spectrophotométrie. Le panneau A montre l’évolution initiale d’une absorbance au cours du temps. Le panneau B présente des droites de Lineweaver-Burk obtenues sans inhibiteur et en présence de trois inhibiteurs X, Y et Z. Le même support sert aux quatre questions suivantes.",
      sharedMedia: {
        type: "image",
