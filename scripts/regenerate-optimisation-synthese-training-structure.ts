...
import { optimisationSyntheseTrainingChapter } from "../prisma/seeds/data/optimisation-synthese-training-chapter";
...
const chapterSlug = optimisationSyntheseTrainingChapter.slug;
const questionSeeds = optimisationSyntheseTrainingChapter.questions;
const sections = optimisationSyntheseTrainingChapter.sections ?? [];
...
main()
  .catch((error) => {
    console.error(
      "Erreur durant la régénération de la structure d'entraînement du chapitre optimisation d'une synthèse :",
      error
    );
    process.exit(1);
  })
