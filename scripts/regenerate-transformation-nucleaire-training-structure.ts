...
import { transformationNucleaireTrainingChapter } from "../prisma/seeds/data/transformation-nucleaire-training-chapter";
...
const chapterSlug = transformationNucleaireTrainingChapter.slug;
const questionSeeds = transformationNucleaireTrainingChapter.questions;
const sections = transformationNucleaireTrainingChapter.sections ?? [];
...
main()
  .catch((error) => {
    console.error(
      "Erreur durant la régénération de la structure d'entraînement du chapitre transformation nucléaire :",
      error
    );
    process.exit(1);
  })
