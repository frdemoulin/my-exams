...
const chapterSlug = lunetteTrainingChapter.slug;
const questionSeeds = lunetteTrainingChapter.questions;
const sections = lunetteTrainingChapter.sections ?? [];
...
main()
  .catch((error) => {
    console.error("Erreur durant la régénération de la structure d'entraînement de la lunette :", error);
    process.exit(1);
  })
  .finally(async () => {
