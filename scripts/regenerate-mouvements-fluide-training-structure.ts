...
import { mouvementsFluideTrainingChapter } from "../prisma/seeds/data/mouvements-fluide-training-chapter";
...
const chapterSlug = mouvementsFluideTrainingChapter.slug;
const questionSeeds = mouvementsFluideTrainingChapter.questions;
const sections = mouvementsFluideTrainingChapter.sections ?? [];
...
main()
  .catch((error) => {
    console.error(
      "Erreur durant la régénération de la structure d'entraînement du chapitre mouvements d'un fluide :",
      error
    );
    process.exit(1);
  })
