...
export function HealthCourseUnitTabs({
  ...
  const initialTabId =
    activeTeachingElementId === 'evaluations' || activeTeachingElementId === 'synthese'
      ? 'evaluations'
      : activeTeachingElementId || courseUnit.teachingElements[0]?.id || 'evaluations';

  const [activeTab, setActiveTab] = useState<string>(initialTabId);

  const isEvaluationsTab = activeTab === 'evaluations' || activeTab === 'synthese';

  return (
    <>
      <div className="flex flex-col gap-6">
        {/* Navigation bar with tabs on the left and 'Aide aux évaluations' on the right on the EXACT same row */}
        <div className="border-b border-default">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div
              ...
              className="flex flex-wrap -mb-px text-sm font-medium text-center text-body"
            >
              {courseUnit.teachingElements.map((teachingElement) => {
                const isActive = activeTab === teachingElement.id;
                return (
                  <button
                    ...
                    onClick={() => setActiveTab(teachingElement.id)}
                    className={`inline-block p-4 border-b-2 font-medium transition-colors focus:outline-none ${
                      isActive
                        ? 'text-fg-brand border-brand font-semibold rounded-t-base'
                        : 'border-transparent text-body hover:text-fg-brand hover:border-brand/40'
                    }`}
                  >
              ...
              <button
                role="tab"
                type="button"
                aria-selected={isEvaluationsTab}
                onClick={() => setActiveTab('evaluations')}
                className={`inline-flex items-center gap-2 p-4 border-b-2 font-medium transition-colors focus:outline-none ${
                  isEvaluationsTab
                    ? 'text-fg-brand border-brand font-semibold rounded-t-base'
                    : 'border-transparent text-body hover:text-fg-brand hover:border-brand/40'
                }`}
            ...
            {/* Aide aux évaluations button ALIGNED TO THE RIGHT ON THE EXACT SAME LINE */}
            {isEvaluationsTab ? (
              <div className="pb-1 sm:pb-0 shrink-0">
                <Popover>
        ...
        {/* Tab panels */}
        {courseUnit.teachingElements.map((teachingElement) => {
          if (activeTab !== teachingElement.id) return null;
          return (
            <div key={teachingElement.id} role="tabpanel" className="space-y-4">
                        ...
                        {HEALTH_COLLES_UE14_V1.map((colle, index) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-4 align-middle">
                              <div className="space-y-1">
                                <div className="flex flex-wrap items-center gap-2">
                                  <span className="font-medium text-heading">{colle.title}</span>
                                  {colle.badgeLabel ? (
                                ...
                                <p className="text-xs italic text-body">{colle.scopeLine}</p>
                                <p className="text-xs text-muted-foreground">{colle.contentLine}</p>
                              </div>
                            </td>
                  ...
                  {courseUnit.mockExams.length > 0 ? (
                    <div className="space-y-3">
                      {courseUnit.mockExams.map((exam) => (
                        <div
                          key={exam.id}
                          className="rounded-xl border border-default bg-card p-4 space-y-4"
                        >
                            ...
                            <div className="text-sm text-muted-foreground">
                              {exam.currentAttemptId ? (
                                ...
                                <span className="inline-flex items-center gap-1.5">
                                  ...
                                  meilleur résultat {exam.bestPercentage}%
                                </span>
                              ) : (
