...
} from "@/core/health-mock-exam/health-mock-exam.scoring";
...
export function HealthMockExamResults({ result, restartHref }: HealthMockExamResultsProps) {
  const answeredQuestionCount = result.questions.filter(
    (question) => question.selectedChoiceIndexes.length > 0,
  ).length;
  const correctQuestionCount = result.questions.filter((question) =>
    areChoiceIndexSetsEqual(question.selectedChoiceIndexes, question.correctChoiceIndexes),
  ).length;
      ...
      <Card className="rounded-base bg-card hover:bg-card">
        <CardHeader className="gap-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-sm text-muted-foreground">Résultats de l&apos;examen blanc</p>
          ...
          return (
            <Card key={question.attemptQuestionId} className="rounded-base bg-card hover:bg-card">
              <CardHeader className="gap-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <Badge variant={isCorrect ? "outline" : "destructive"}>
                    {isCorrect ? "Correcte" : question.selectedChoiceIndexes.length === 0 ? "Sans réponse" : "À revoir"}
                  </Badge>
                </div>
                {question.group ? (
              ...
              <CardContent className="space-y-4">
                ...
                <p className="text-sm text-muted-foreground">Votre réponse : {formatChoiceLetters(question.selectedChoiceIndexes)} · Réponse attendue : {formatChoiceLetters(question.correctChoiceIndexes)}</p>
              </CardContent>
            </Card>
          );
        })}
