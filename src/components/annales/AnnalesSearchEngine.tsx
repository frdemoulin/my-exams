...
export interface AnnalesSearchEngineProps {
  initialSubjects: Subject[];
  initialDiplomas: Diploma[];
  initialDiplomaFilter?: string;
...
export function AnnalesSearchEngine({
  initialSubjects,
  initialDiplomas,
  initialDiplomaFilter,
  initialSubjectFilter,
  initialSearchQuery,
  autoSearchOnMount = false,
  headerTitle,
  headerDescription,
}: AnnalesSearchEngineProps) {
  ...
  const [selectedDiploma, setSelectedDiploma] = useState<string | undefined>(initialDiplomaFilter);
  const [selectedSubject, setSelectedSubject] = useState<string | undefined>(initialSubjectFilter);
  ...
  const [showResults, setShowResults] = useState(Boolean(autoSearchOnMount || initialDiplomaFilter || initialSubjectFilter || initialSearchQuery));
  const [hasUserSearched, setHasUserSearched] = useState(Boolean(autoSearchOnMount || initialDiplomaFilter || initialSubjectFilter || initialSearchQuery));
  ...
  return (
    <div className="space-y-6">
      ...
      {(headerTitle || headerDescription) && (
        <div className="space-y-2">
          {headerTitle && <h1 className="text-2xl font-bold tracking-tight text-heading md:text-3xl">{headerTitle}</h1>}
          {headerDescription && <p className="text-sm text-muted-foreground md:text-base">{headerDescription}</p>}
        </div>
      )}
