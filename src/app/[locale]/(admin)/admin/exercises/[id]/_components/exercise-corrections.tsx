...
import { createExerciseCorrection, deleteExerciseCorrection } from "@/core/exercise-correction";
...
interface ExerciseCorrectionItem {
  id: string;
  source: string;
  url: string;
  type: string;
...
interface ExerciseCorrectionsProps {
  exerciseId: string;
  corrections: ExerciseCorrectionItem[];
...
export const ExerciseCorrections = ({
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await createExerciseCorrection(formData);
  ...
  const handleDelete = async (correctionId: string) => {
    ...
    try {
      await deleteExerciseCorrection(correctionId, { exerciseId });
      ...
      <CardContent className="space-y-4">
        {corrections.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            Aucune correction enregistrée pour cet exercice.
          </p>
        ) : (
        ...
        <div className="rounded-lg border border-dashed border-border p-4">
          ...
          {activeSources.length === 0 ? (
            ...
            <form
              ...
              className="mt-4 grid gap-3 md:grid-cols-[1fr_2fr_1fr_auto]"
            >
              <div>
