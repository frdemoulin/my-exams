...
const ViewExamPaperPage = async ({
  ...
  const correctionSources = await fetchCorrectionSources({
  ...
  const pendingCount = exercises.filter(
    (ex) => ex.enrichmentStatus === "pending",
  ).length;
  const failedCount = exercises.filter(
    (ex) => ex.enrichmentStatus === "failed",
  ).length;
  ...
  const subjectUrlIsExternal = isExternalUrl(
    examPaper.subjectUrl,
    internalOrigin,
  );
  const sourceUrlIsExternal = isExternalUrl(
    examPaper.sourceUrl,
    internalOrigin,
  );
  const normalizedLabel =
    normalizeExamPaperLabel(examPaper.label) ?? examPaper.label;
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            {examPaper.curriculum && (
              <div>
                ...
                <p className="text-sm">
                  {examPaper.curriculum.shortDescription ||
                    examPaper.curriculum.longDescription}
                </p>
              </div>
            )}
            {examPaper.examinationCenters &&
              examPaper.examinationCenters.length > 0 && (
                <div>
                  <h3 className="text-sm font-semibold text-muted-foreground">
                    Centre d&apos;examen
                  </h3>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold">
              {exercisesWithThemes.length} exercice
              {exercisesWithThemes.length > 1 ? "s" : ""}
            </h2>
            <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-baseline justify-between">
                          ...
                          {(() => {
                            ...
                            const labelPart =
                              ...
                              !exercise.label
                                .trim()
                                .toLowerCase()
                                .startsWith(baseTitle.toLowerCase())
                                ? ` - ${exercise.label}`
                                : "";
                        ...
                        <div className="flex gap-2">
                          ...
                          {exercise.estimatedDifficulty && (
                            <Badge variant="difficulty">
                              Difficulté {exercise.estimatedDifficulty}/5
                            </Badge>
                          )}
                          <Badge
                            variant={
                          ...
                          >
                            {exercise.enrichmentStatus === "completed" &&
                              "Enrichi"}
                            {exercise.enrichmentStatus === "pending" &&
                              "En attente"}
                      ...
                      <div className="flex gap-1">
                        <ReenrichExerciseButton
                          ...
                          enrichmentStatus={normalizeEnrichmentStatus(
                            exercise.enrichmentStatus,
                          )}
                        />
                        <TooltipProvider>
                          <Tooltip>
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              ...
                              <TooltipContent className="max-w-xs text-xs font-normal leading-relaxed">
                                Résumé généré avec l&apos;aide d&apos;une
                                intelligence artificielle et relu par
                                l&apos;équipe My Exams. Résumé indicatif,
