# Configuration i18n avec next-intl

Ce document explique comment utiliser le système d'internationalisation (i18n) mis en place avec next-intl dans le projet.

## 📁 Structure

```
src/
├── i18n/
│   ├── routing.ts              # Configuration des routes et locales
│   ├── request.ts              # Configuration pour le serveur
│   ├── config.ts               # Configuration globale
│   ├── middleware-config.ts    # Config pour le middleware
│   └── messages/
│       ├── fr.json            # Traductions françaises
│       └── en.json            # Traductions anglaises
├── middleware.ts               # Middleware Next.js pour i18n
└── hooks/
    └── use-translations.ts     # Hooks personnalisés
```

## 🌍 Locales supportées

- **Français (fr)** - Locale par défaut
- **Anglais (en)**

## 🎯 Utilisation dans les composants

### 1. Traductions des entités métier

```tsx
'use client';

import { useEntityTranslation } from '@/hooks/use-translations';

export function DiplomaPage() {
  const entity = useEntityTranslation('diploma');
  
  return (
    <div>
      <h1>{entity.plural}</h1>
      <button>{entity.actions.add}</button>
    </div>
  );
}
```

### 2. Traductions communes

```tsx
'use client';

import { useCommonTranslations } from '@/hooks/use-translations';

export function Form() {
  const common = useCommonTranslations();
  
  return (
    <div>
      <button>{common.save}</button>
      <button>{common.cancel}</button>
    </div>
  );
}
```

### 3. Messages de succès/erreur

```tsx
'use client';

import { useMessageTranslations } from '@/hooks/use-translations';
import { toast } from 'react-hot-toast';

export function useDiplomaActions() {
  const messages = useMessageTranslations();
  
  const createDiploma = async (data: any) => {
    try {
      // ... logique de création
      toast.success(messages.success.created('Diplôme'));
    } catch (error) {
      toast.error(messages.error.generic);
    }
  };
  
  return { createDiploma };
}
```

### 4. Traduction directe avec next-intl

```tsx
'use client';

import { useTranslations } from 'next-intl';

export function CustomComponent() {
  const t = useTranslations('common');
  
  return <button>{t('save')}</button>;
}
```

### 5. Côté serveur

```tsx
import { getTranslations } from 'next-intl/server';

export default async function ServerPage() {
  const t = await getTranslations('common');
  
  return <h1>{t('welcome')}</h1>;
}
```

## 🔄 Changement de langue

Le composant `LocaleSwitcher` permet de changer de langue :

```tsx
import { LocaleSwitcher } from '@/components/shared/locale-switcher';

export function Navbar() {
  return (
    <nav>
      {/* ... autres éléments */}
      <LocaleSwitcher />
    </nav>
  );
}
```

## 🔗 Navigation avec i18n

Utilisez les utilitaires de routing de next-intl au lieu de ceux de Next.js :

```tsx
// ❌ N'utilisez pas
import Link from 'next/link';
import { useRouter } from 'next/navigation';

// ✅ Utilisez plutôt
import { Link, useRouter } from '@/i18n/routing';

export function Navigation() {
  const router = useRouter();
  
  return (
    <div>
      <Link href="/dashboard">Dashboard</Link>
      <button onClick={() => router.push('/admin')}>Admin</button>
    </div>
  );
}
```

## 📝 Ajouter de nouvelles traductions

### 1. Ajoutez les clés dans les fichiers JSON

**src/i18n/messages/fr.json**
```json
{
  "myNewSection": {
    "title": "Mon titre",
    "description": "Ma description"
  }
}
```

**src/i18n/messages/en.json**
```json
{
  "myNewSection": {
    "title": "My title",
    "description": "My description"
  }
}
```

### 2. Utilisez-les dans vos composants

```tsx
import { useTranslations } from 'next-intl';

export function MyComponent() {
  const t = useTranslations('myNewSection');
  
  return (
    <div>
      <h1>{t('title')}</h1>
      <p>{t('description')}</p>
    </div>
  );
}
```

## 🎨 Entités disponibles

Les entités suivantes sont traduites dans `entities.*` :

- `diploma` - Diplôme
- `division` - Filière
- `examinationCenter` - Centre d'examen
- `grade` - Niveau scolaire
- `subject` - Matière
- `theme` - Thème
- `user` - Utilisateur

Chaque entité a :
- `singular` / `plural`
- `article.definite` / `article.indefinite`
- `actions.add` / `actions.edit` / `actions.delete` / `actions.view` / `actions.list`

## 🔍 URLs et SEO

next-intl gère automatiquement les URLs localisées :

- **Français (défaut)** : `/dashboard`, `/admin/diplomas`
- **Anglais** : `/en/dashboard`, `/en/admin/diplomas`

La locale par défaut (français) n'ajoute pas de préfixe dans l'URL grâce à `localePrefix: 'as-needed'`.

## 🚀 Bonnes pratiques

1. **Toujours utiliser les hooks** plutôt que d'accéder directement aux messages
2. **Grouper les traductions** par contexte (common, auth, navigation, etc.)
3. **Éviter les hardcoded strings** dans l'UI - toujours passer par i18n
4. **Tester les deux langues** lors du développement de nouvelles fonctionnalités
5. **Utiliser des clés descriptives** plutôt que des raccourcis

## 🛠️ Configuration technique

### Middleware

Le middleware intercepte toutes les requêtes et gère automatiquement :
- Détection de la locale
- Redirection vers la bonne URL localisée
- Gestion des cookies de préférence

### next.config.mjs

Le plugin next-intl est configuré pour charger les messages au build :

```js
import createNextIntlPlugin from 'next-intl/plugin';
const withNextIntl = createNextIntlPlugin('./src/i18n/request.ts');

export default withNextIntl(nextConfig);
```

## 📚 Ressources

- [Documentation next-intl](https://next-intl-docs.vercel.app/)
- [Next.js i18n routing](https://nextjs.org/docs/app/building-your-application/routing/internationalization)
